...
export default function ReservationCalendarApp() {
	...
	async function reloadEntries(userId) {
		...
		const normalizedItems = rawItems.map((item) => ({
			...item,
			[FIELD_CENTER]: resolveEnumLabel(enumMaps, FIELD_CENTER, item[FIELD_CENTER]),
			...
			[FIELD_STATUS]: resolveEnumLabel(enumMaps, FIELD_STATUS, item[FIELD_STATUS]),
		}));
	...
	useEffect(() => {
		...
		window.BX24.init(async () => {
			try {
				...
				setCurrentUser(user);
				setEnumMaps(createEnumMaps(fieldsResponse?.fields || {}));

				await reloadEntries(user.ID);
				setLoading(false);
			} catch (err) {
				console.error(err);
						...
						<div className="flex flex-col gap-2">
							...
							{selectedEntry ? (
								...
								<>
									...
									<p className="text-xs text-slate-500">
										Pulsa en el día para crear la jornada.
									</p>
								</>
							)}
							...
							{currentUser ? (
								<p className="mt-2 text-xs text-slate-500">
									Usuario actual: {currentUser.NAME} {currentUser.LAST_NAME}
								</p>
							) : null}

							<p className="text-xs text-slate-500">
								Registros deduplicados del mes visible: {visibleMonthEntries.length}
							</p>
						</div>
					</div>
				) : null}
